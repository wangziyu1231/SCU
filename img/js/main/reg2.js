$(function(){
			...
			$('input[name="realname"]').focus(function(){
				$(this).next().text('真实姓名').css("color","#000");//.addClass('state2');
			})
			.blur(function(){
				if($(this).val()==''){
					$(this).next().text('姓名不能为空').css("color","	#FF0000");
				}else if($(this).val().search(/[\u4e00-\u9fa5]/)==-1){
			...
			$('input[name="major"]').focus(function(){
				$(this).next().text('请填写代码').css("color","#000");
			})
			.blur(function(){
				if($(this).val()==''){
					$(this).next().text('专业不能为空').css("color","	#FF0000");
				}else if($(this).val().search(/^\d{5}/)==-1){
			...
			.blur(function(){
				...
				}else if($(this).val().search(/13[0-9]{9}/)==-1){
